...
The mean and median values are close in value ignoring the NA values:  
  ...
  Mean: $`r round(mean(stepcount$totalsteps), 1)`$  
  Median : $`r round(median(stepcount$totalsteps),1)`$
...
The data has NA values for `r round(mean(is.na(activitydata$steps)), 3)*100`% of the days. A total of `r str_sub(summary[7,1], -6, -3)` rows out of a total of `r length(activitydata$steps)` are missing (NA's). The missing data will be imputed by using the average of each time interval dependent on if the day falls during the week or on a weekend. 
...
The mean value is a little lower and the median value dropped more significantly with the imputed values. Imputing the values with this method averaged out the overall data:  
  ...
  Mean: $`r round(mean(stepimputed$totalsteps), 1)`$  
  Median : $`r round(median(stepimputed$totalsteps),1)`$
